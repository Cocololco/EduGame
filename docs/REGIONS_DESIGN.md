...
**Status: implemented**, including three follow-up extensions past the original spec — **multi-factory manufacturing** (a product can run factories in more than one country at once, not just relocate between them), **price per country** (real price discrimination, not one flat price everywhere), and **multi-country market research** (any number of countries in one year, not just one). This was originally written as a spec before any of it was built (see git history if you want the "not yet built" version); it now describes the shipped system — engine (`src/lib/simulation/countries.ts`, `simulateYear.ts`), decisions/UI (`decisionOptions.ts`, `CompanyDecisionPanel`/`ProductDecisionPanel`/`CompanyStatusPanel`), and bots (`botAi.ts`, licenses only — see that file for why bots skip research, extra factories, and per-country pricing). The one piece still deferred per the build order below: a per-country breakdown on the financials page (it currently only breaks down by product).
...
8. ⬜ **Still deferred**: a per-country breakdown on the financials page — it only breaks down by product today. Worth doing once it's clear players actually want to see it broken out that way, rather than speculatively.
