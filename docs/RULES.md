...
- **Price per country**: a product's `price` is the *default*, used everywhere you don't set something more specific — but you can override it per licensed country (real price discrimination). Each country's own price drives its own demand there (same price-elasticity formula, that country's own number), and revenue is the demand-weighted average of whatever prices actually applied.
- **Factory** (per product, one-time per country the *first* time any product opens one there — opening where the company already manufactures anything, for this or another product, is free): **opens an ADDITIONAL manufacturing base, on top of any this product already runs** — a product can have factories running in more than one country at once. All of a product's factories still share ONE pool of employees/wages/physical capacity (see "Wages & training" above) — what changes per factory is only how much of that shared output is attributed to it, which sets the wage bill's labor-cost multiplier: `wagesExpense = employees × wageLevel × (Σ each factory's share of this year's production × its country's labor multiplier)`. A 50/50 split between a ×1.4 and a ×0.5 country charges the blended ×0.95 rate, not either extreme.
- **Transport cost**: a flat **+$5/unit** surcharge on COGS for every unit sold into a country with no open factory for that product (blended proportionally to how much of a solo game's demand comes from each licensed country; in multiplayer it's the same $5/unit on whichever units cross a border).
- **Market research** (company-wide, **any number of countries in the same year**): reveals each researched country's price/quality/brand/innovation weights in the company-status table. **UI-only** — the simulation always uses the real weights whether you've researched a country or not; research just lets you *see* them to plan deliberately instead of learning by trial and error.
...
- **Solo mode**: demand sums across every country you're licensed in (`baseDemandUnits × country's effective per-product demand multiplier × attractiveness`, computed separately per country using that country's own price — weight-blending itself stays a multiplayer-only nuance, solo uses the continuous attractiveness formula throughout).
- **Multiplayer**: the category-leadership system (above) runs **per country per product**, among only the players licensed there, using each player's price *for that country* — a player who hasn't licensed a country neither competes for nor dilutes anyone else's share of its demand pool.
...
- **Standard**: the full decision set above, except international expansion. Production volume is still shown per-factory under the hood, but there's only ever one factory (France) below Advanced, so it renders as the same single field as always.
- **Advanced**: same as Standard, plus international expansion — per-country pricing and opening additional factories (per product), licenses and market research (company-wide, research allows any number of countries per year).
