...
What *is* part of state: `ProductLineState.factoryCountries` (every country this product currently manufactures in — a product can run more than one factory at once, all sharing its single employees/wageLevel/productionCapacity pool) and `CompanyYearState.licensedCountries` / `openedFactoryCountries` / `researchedCountries` (company-wide lists, start as `["france"]`).

Decisions: `ProductDecision.openFactoryIn?: CountryId` ADDS a factory (on top of, not instead of, any this product already runs); `ProductDecision.price: number` is the default price and `ProductDecision.priceByCountry?: Partial<Record<CountryId, number>>` overrides it per licensed country (real price discrimination — revenue is the demand-weighted average of whatever prices actually applied); `ProductDecision.productionVolumeByFactory: Partial<Record<CountryId, number>>` replaces the old flat `productionVolume` — one entry per open factory, their combined total still capped by the shared capacity/labor pool; `CompanyDecision.licenseCountry?: CountryId` (one per year) and `CompanyDecision.researchCountries?: CountryId[]` (any number in the same year, cost is per country) are company-wide. All of these are one-time (not annual) purchases that land in the *closing* state, same "pays off next year" rule as everything else.
