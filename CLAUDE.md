...
**International expansion is implemented** (selling into multiple countries, factories, transport cost, licenses, paid market research — see [docs/REGIONS_DESIGN.md](docs/REGIONS_DESIGN.md) for the full mechanics, now marked as built) and gated to the **Advanced** difficulty tier. The one deferred piece: the financials page has no per-country breakdown yet, only per-product.
